...
//set the timeout to 50ms (makes "receive" pseudo-non-blocking) 
			socket.setSoTimeout(50);
//prepare the required readers and prepare a (re-used) packet buffer 
			DatagramPacket packet = new DatagramPacket(new byte[512], 512);
			int id = buildDNSQuery(packet, "www.hs-kempten.de");
			packet.setPort(53);
			packet.setAddress(InetAddress.getByAddress(new byte[] { (byte) 8, 8, 8, 8 }));
//send the prepared packet
			System.out.println("--- Contacting server with query id " + id);
			socket.send(packet);
//reset length of buffer for receive
			packet.setLength(512);
			int waits = 0;
			while (true) {
				try {
					socket.receive(packet); // try to receive a packet
					evaluateDNSAnswer(packet.getData(), packet.getLength()); // get data
					break;
				} catch (SocketTimeoutException e) { // timeout: print a dot
					System.out.print('.');
					if (++waits > 50) {
	...
	private static int buildDNSQuery(DatagramPacket packet, String query) {
		...
		packet.setLength(0);
		/* header */
		appendShort(packet, id = (int) (Math.random() * 65536)); // generate random id 
		appendShort(packet, 0x0100);
																	// //flags
...
//name consists of labels, each label has 8 bit length and label string 
		...
		for (String label : labels) {
			...
			for (int i = 0; i < labelLen; i++)
				appendByte(packet, label.charAt(i));
		}
...
//packet is built, return query id
		return id;
	}
...
//check header: flags should indicate "answer" and "no error"
		int flags = getShort(data, 2);
		if ((flags & 0x8000) == 0 || (flags & 0x000F) != 0) {
			System.out.println("flags indicate invalid answer: " + Integer.toHexString(flags));
			return;
		}
		int qdcount = getShort(data, 4), ancount = getShort(data, 6);
...
//read answers
		while (ancount-- > 0)
			pos = evaluateDNSAnswerRecord(data, pos);
	}

	private static int evaluateDNSAnswerRecord(byte[] buf, int pos) { /* resource */
		System.out.println("--- answer record ---");
		pos = skipName(buf, pos); // skip name
		System.out.println("type: " + getShort(buf, pos));
		pos += 4; // type and class
		System.out.println("ttl: " + getInt(buf, pos) / 60 + " minutes");

		pos += 4; // ttl
		int rdlen = getShort(buf, pos);
		pos += 2;
		System.out.print(rdlen + " data bytes: ");
		while (rdlen-- > 0)
			System.out.print(getByte(buf, pos++) + " ");
		System.out.println();
		return pos;
	}
	...
	private static int getShort(byte[] buf, int pos) {
		...
		return (getByte(buf, pos) << 8) + getByte(buf, pos + 1);
	}

	private static int getInt(byte[] buf, int pos) {
		// return big endian short
		return (getShort(buf, pos) << 16) + getShort(buf, pos + 2);
	}
	...
	private static int skipName(byte[] buf, int pos) {
		...
		do {
			...
			if (labelLen == 0)
				anotherLabel = false; // end of label
			else if ((labelLen & 0xC0) == 0xC0) { // label pointer with offset
				pos++; // skip over offset of label pointer
				anotherLabel = false;
			} else
				pos += labelLen; // skip over name of label
		} while (anotherLabel);
		return pos;
	}
}
